...
export default function Entrar() {
  ...
  const enviar = async () => {
    setError(null);
    if (!/.+@.+\..+/.test(correo)) return setError("Escribe un correo valido.");
    if (clave.length < 8) return setError("La contrasena necesita al menos 8 caracteres.");
    setCargando(true);
    ...
    setCargando(false);
    if (e) return setError(modo === "crear" ? "No se pudo crear la cuenta. Puede que el correo ya exista." : "Correo o contrasena incorrectos.");
  ...
  return (
    <main className="flex min-h-dvh flex-col justify-center gap-4 px-6">
      ...
      <p className="text-sm text-mist">Entras con seudonimo, nadie sabe quien eres.</p>
      ...
      <p className="text-[11px] leading-relaxed text-faint">
        Al crear tu cuenta aceptas que SafeSpace es apoyo entre pares y no sustituye atencion profesional.
        El boton SOS conecta en todo momento con lineas de atencion.
      </p>
    </main>
  );
}
